...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	const getUSyncDevices = async (
		...
		if (result) {
			...
			const processUserDevices = (
				user: string,
				userDevices: JidWithDevice[],
				requestedLidUsers: Set<string>,
				deviceResults: DeviceWithJid[]
			) => {
	...
	const relayMessage = async (
		...
		if (isLid) {
			...
			if (pnJid) {
				...
				logger.error(
					{ jid },
					'Failed to convert LID to PN for message relay. Messages to this JID might not be delivered.'
				)
